...
export const CreateAuction: React.FC = () => {
  ...
  const handleChange = (
    ...
    if (name === 'fechaInicio') {
      const selectedDate = parseLocalInputDate(value);
      const now = new Date();
      // Si la fecha seleccionada es más de 1 minuto anterior al momento actual
      if (selectedDate.getTime() < now.getTime() - 60000) {
        setFechaInicioError('Debes elegir una fecha y hora posterior a la actual.');
      } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (inicioDate.getTime() < now.getTime() - 60000) {
      const msgFechaPasada = 'Debes elegir una fecha y hora posterior a la actual para el inicio.';
      setFechaInicioError(msgFechaPasada);
      setError(msgFechaPasada);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 font-sans">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label
                  htmlFor="fechaInicio"
                  className="block text-xs sm:text-sm font-semibold text-slate-700 font-sans"
                >
                ...
                <button
                  type="button"
                  onClick={() => {
                    const nowStr = toLocalDatetimeInputString(new Date());
                    setFormData((prev) => ({ ...prev, fechaInicio: nowStr }));
                    setFechaInicioError(null);
                  }}
                  className="text-[11px] font-semibold text-brand-action hover:underline cursor-pointer"
                >
                  Fijar fecha actual
                </button>
              </div>
              <div className="relative">
                <input
                  ...
                  type="datetime-local"
                  name="fechaInicio"
                  min={toLocalDatetimeInputString(new Date())}
                  value={formData.fechaInicio}
                  onChange={handleChange}
                  disabled={isSubmitting}
                  className={`w-full px-3.5 py-2.5 text-sm sm:text-base font-medium rounded-lg bg-white border ${
                    fechaInicioError
                      ? 'border-rose-500 text-rose-900 focus:border-rose-500 focus:ring-2 focus:ring-rose-500/20'
                      : 'border-slate-300 text-brand-dark focus:border-brand-action focus:ring-2 focus:ring-brand-action/15'
                  } transition-all disabled:bg-slate-50 font-sans cursor-pointer outline-none`}
                  required
                />
              </div>

              {/* Alerta si la fecha de inicio es pasada */}
              {fechaInicioError ? (
                <div className="flex items-center gap-1.5 text-xs text-rose-600 font-semibold mt-1.5 animate-in fade-in slide-in-from-top-1 duration-150">
                  <AlertCircle className="w-4 h-4 shrink-0 text-rose-500" />
                  <span>{fechaInicioError}</span>
                </div>
              ) : (
                <span className="text-xs text-slate-400 mt-1.5 block font-sans">
            ...
            <div>
              ...
              <input
                ...
                type="datetime-local"
                name="fechaFin"
                min={formData.fechaInicio || toLocalDatetimeInputString(new Date())}
